...
const EditBudget = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        toast.showToast("Failed to load categories", "error");
      }
    };
  ...
  const executeDelete = async () => {
    try {
      ...
      setTimeout(() => {
        toast.showToast(
          error?.message || "Cannot delete budget. Please try again.",
          "error"
        );
      }, 300);
    }
  ...
  const handleUpdateBudget = async () => {
    try {
      // Validate inputs
      if (!name.trim()) {
        toast.showToast("Please enter a budget name", "error");
        return;
      }
      ...
      if (!hasChanges) {
        toast.showToast("No changes to update", "warning");
        return;
      }
      ...
      toast.showToast(error?.message || "Failed to update budget", "error");
    } finally {
      setLoading(false);
    }
  };
